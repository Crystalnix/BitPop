...
ChatPopup::ChatPopup(ExtensionHost* host,
                     views::Widget* frame,
...
void ChatPopup::Show(bool activate) {
  if (popup_->IsVisible())
    return;
...
void ChatPopup::BubbleBrowserWindowMoved(BrowserBubble* bubble) {
  ResizeToView();
}

void ChatPopup::BubbleBrowserWindowClosing(BrowserBubble* bubble) {
  if (!closing_)
    Close();
}

void ChatPopup::BubbleGotFocus(BrowserBubble* bubble) {
  // Forward the focus to the renderer.
  host()->render_view_host()->view()->Focus();
}

void ChatPopup::BubbleLostFocus(BrowserBubble* bubble,
...
void ChatPopup::Observe(NotificationType type,
  ...
  switch (type.value) {
    case NotificationType::EXTENSION_HOST_DID_STOP_LOADING:
      ...
      if (extension_host_.get() == Details<ExtensionHost>(details).ptr()) {
        gfx::Rect rc = extension_host_->view()->bounds();
...
ChatPopup* ChatPopup::Show(
  ...
  ChatPopup* popup = new ChatPopup(host, frame, relative_to,
                                             arrow_location,
                                             observer);
...
void ChatPopup::Close() {
  ...
  if (observer_)
    observer_->ChatPopupIsClosing(this);
