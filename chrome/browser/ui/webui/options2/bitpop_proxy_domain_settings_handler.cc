...
void BitpopProxyDomainSettingsHandler::OnItemsChanged(int start, int length) {
  OnModelChanged();
}

void BitpopProxyDomainSettingsHandler::OnItemsAdded(int start, int length) {
  OnModelChanged();
}

void BitpopProxyDomainSettingsHandler::OnItemsRemoved(int start, int length) {
  OnModelChanged();
}

base::DictionaryValue* BitpopProxyDomainSettingsHandler::CreateDictionaryForExtension(
...
base::DictionaryValue* BitpopProxyDomainSettingsHandler::CreateDictionaryForEngine(
...
void BitpopProxyDomainSettingsHandler::SetDefaultSearchEngine(const ListValue* args) {
  int index;
...
void BitpopProxyDomainSettingsHandler::RemoveSearchEngine(const ListValue* args) {
  int index;
  if (!ExtractIntegerValue(args, &index)) {
    NOTREACHED();
    return;
...
void BitpopProxyDomainSettingsHandler::EditSearchEngine(const ListValue* args) {
  int index;
  if (!ExtractIntegerValue(args, &index)) {
    NOTREACHED();
    return;
...
void BitpopProxyDomainSettingsHandler::OnEditedKeyword(
...
void BitpopProxyDomainSettingsHandler::CheckSearchEngineInfoValidity(
...
void BitpopProxyDomainSettingsHandler::EditCancelled(const ListValue* args) {
  if (!edit_controller_.get())
    return;
...
void BitpopProxyDomainSettingsHandler::EditCompleted(const ListValue* args) {
  if (!edit_controller_.get())
    return;
  string16 name;
  string16 keyword;
