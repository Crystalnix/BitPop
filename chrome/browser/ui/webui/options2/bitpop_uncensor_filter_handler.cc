...
void BitpopUncensorFilterHandler::RegisterMessages() {
  ...
  web_ui()->RegisterMessageCallback(
      "removeSearchEngine",
      base::Bind(&BitpopUncensorFilterHandler::RemoveSearchEngine,
                 base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "editSearchEngine",
      base::Bind(&BitpopUncensorFilterHandler::EditSearchEngine,
                 base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "checkSearchEngineInfoValidity",
      base::Bind(&BitpopUncensorFilterHandler::CheckSearchEngineInfoValidity,
                 base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "searchEngineEditCancelled",
      base::Bind(&BitpopUncensorFilterHandler::EditCancelled,
                 base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "searchEngineEditCompleted",
      base::Bind(&BitpopUncensorFilterHandler::EditCompleted,
                 base::Unretained(this)));
}
...
void BitpopUncensorFilterHandler::OnItemsChanged(int start, int length) {
  OnModelChanged();
}

void BitpopUncensorFilterHandler::OnItemsAdded(int start, int length) {
  OnModelChanged();
}

void BitpopUncensorFilterHandler::OnItemsRemoved(int start, int length) {
  OnModelChanged();
}

base::DictionaryValue* BitpopUncensorFilterHandler::CreateDictionaryForExtension(
...
base::DictionaryValue* BitpopUncensorFilterHandler::CreateDictionaryForEngine(
...
void BitpopUncensorFilterHandler::SetDefaultSearchEngine(const ListValue* args) {
  int index;
...
void BitpopUncensorFilterHandler::RemoveSearchEngine(const ListValue* args) {
  int index;
  if (!ExtractIntegerValue(args, &index)) {
    NOTREACHED();
    return;
...
void BitpopUncensorFilterHandler::EditSearchEngine(const ListValue* args) {
  int index;
  if (!ExtractIntegerValue(args, &index)) {
    NOTREACHED();
    return;
...
void BitpopUncensorFilterHandler::OnEditedKeyword(
...
void BitpopUncensorFilterHandler::CheckSearchEngineInfoValidity(
...
void BitpopUncensorFilterHandler::EditCancelled(const ListValue* args) {
  if (!edit_controller_.get())
    return;
...
void BitpopUncensorFilterHandler::EditCompleted(const ListValue* args) {
  if (!edit_controller_.get())
    return;
  string16 name;
  string16 keyword;
